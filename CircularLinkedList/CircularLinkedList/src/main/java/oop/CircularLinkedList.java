...
public class CircularLinkedList {
    ...
    public void enqueue(int value) {
        ...
        if (isEmpty()){
            ...
            node.setNext(getFirst().get());
            getLast().get().setNext(node);
            this.last = Optional.of(node);
        }
        this.size += 1;
    ...
    public int remove(int index){
        Node node_apres = getFirst().get();
        Node node_avant = getLast().get();
        for(int i = 0; i<index; i++){
            node_avant = node_avant.next.get();
            node_apres = node_apres.next.get();
        }
        node_avant.setNext(node_apres.next.get());
