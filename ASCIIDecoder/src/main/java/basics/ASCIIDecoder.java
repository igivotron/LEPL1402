...
public class ASCIIDecoder {
    ...
    public static String [] decode(int[] forbidden, String[][] sentences){
        ...
        if (forbidden != null) for (int fb : forbidden) f.add(fb);
        String[] theForbiddenQuote = new String[sentences.length];
        int i =0;
        for(String[] x:sentences){
            StringBuilder bob = new StringBuilder();
            for (String y: x) if (!f.contains(Integer.parseInt(y))) bob.appendCodePoint(Integer.parseInt(y));
            theForbiddenQuote[i] = bob.toString();
            i++;
        }
        return theForbiddenQuote;
    }
