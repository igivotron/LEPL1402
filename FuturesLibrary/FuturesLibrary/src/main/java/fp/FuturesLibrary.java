...
public class FuturesLibrary {
    ...
    public static int countMatchingBooks(Library library,
        ...
        for (int i=startBookIndex; i<countBooks+startBookIndex; i++){
    ...
    public static int countMatchingBooksWithThreads(Library library,
        ...
        if (countThreads == 0) {throw new IllegalArgumentException();}
        Stack<Future<Integer>> futures = new Stack<>();
        int n;
        if (library.getNumberOfBooks() % countThreads == 0){n = library.getNumberOfBooks() / countThreads;}
        else {n= library.getNumberOfBooks() / countThreads +1;}
        for(int i = 0; i<countThreads; i++){
            final int start = i*n;
            final int c = Math.min(n, library.getNumberOfBooks()-start);
            futures.add(executor.submit(() -> countMatchingBooks(library, predicate, start, c)));
        }
