...
public class BrickCounter {
    ...
    public static int[] countBricks(Brick[] bricks, int n, Function<Brick, Integer> sorter) {
        int[] box = new int[n];
        for(Brick i:bricks){box[sorter.apply(i)] ++;}
        return box;
    }
    ...
    public static int[] countBricksTwoThreads(Brick[] bricks, int n, Function<Brick, Integer> sorter, ExecutorService executor) {
        int l = bricks.length;
        int[] b = new int[n];
        Brick[] b1 = Arrays.copyOfRange(bricks, 0, l/2);
        Brick[] b2 = Arrays.copyOfRange(bricks, l/2, l);
        Future<int[]> Alice = executor.submit(() -> countBricks(b1, n, sorter));
        Future<int[]> Bob = executor.submit(() -> countBricks(b2, n, sorter));
        try {
            int[] b1count = Alice.get();
            int [] b2count = Bob.get();
            for(int i=0; i<n; i++){
                b[i] = b1count[i] + b2count[i];
            }
            return b;
